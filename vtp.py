...
# ===== CZĘŚĆ 1: KONWERSJA NRRD DO VTP =====
def convert_nrrd_to_vtp(input_path, output_path="output.vtp"):
...
# ===== CZĘŚĆ 2: ANALIZA CENTERLINE =====
def analyze_centerline(vtp_path):
    ...
    def find_smart_centerline(G, lookahead=5):
        ...
        while True:
            ...
            for candidate in neighbors:
                ...
                if candidate_norm == 0:
                    continue
                cos_angle = 1.0 if forward_vec is None else np.dot(forward_vec, candidate_vec) / (
                            np.linalg.norm(forward_vec) * candidate_norm)
    ...
    # === Parametry wykrywania zwężeń
    derivative_threshold = 0.045  # Zmieniaj ten próg, np. 0.03–0.1
    min_narrowing_length = 6  # Minimalna liczba punktów w zwężeniu

    # === Detekcja zwężeń
    smoothed_coords = smooth_path(ordered_coords)

    def detect_narrowings(centerline_indices, distances, derivative_threshold=0.045, min_length=3):
        radii = np.array([distances[i] for i in centerline_indices])
        deriv = np.gradient(radii)
        ...
        for i in range(1, len(deriv) - 1):
            if not in_narrowing and deriv[i] < -derivative_threshold and deriv[i - 1] >= deriv[i]:
                start_idx = i
                in_narrowing = True
            elif in_narrowing and deriv[i] > derivative_threshold and deriv[i] >= deriv[i - 1]:
                end_idx = i
                if end_idx - start_idx >= min_length:
                    narrowings.append((start_idx, end_idx))
                in_narrowing = False

        return narrowings

    narrowing_segments = detect_narrowings(path_indices, distances, derivative_threshold, min_narrowing_length)

    # Tworzymy maskę dla obszarów zwężenia
    narrowing_points = set()
    for start, end in narrowing_segments:
        for idx in path_indices[start:end + 1]:
            narrowing_points.add(idx)

    narrowing_mask = np.zeros(len(coords_mm), dtype=np.uint8)
    centerline_idx_array = np.array(list(path_indices))
    for start, end in narrowing_segments:
        segment_indices = centerline_idx_array[start:end + 1]
        narrowing_mask[segment_indices] = 1
    ...
    skeleton_cloud["narrowing"] = narrowing_mask

    plotter = pv.Plotter()
    plotter.add_mesh(mesh, color="lightblue", opacity=0.2, label="Aorta")
    plotter.add_mesh(skeleton_cloud, scalars="distance_to_wall", cmap="plasma", point_size=2,
                     render_points_as_spheres=True, label="Skeleton")
    plotter.add_mesh(centerline_polyline, color="darkred", line_width=6,
                     label="Smoothed Centerline")

    # Dodajemy chmurę punktów tylko jeśli istnieją zwężenia
    narrowing_cloud = skeleton_cloud.threshold(value=0.5, scalars="narrowing")
    if narrowing_cloud.n_points > 0:  # Sprawdzamy czy są punkty zwężeń
        plotter.add_mesh(narrowing_cloud, color="yellow", point_size=6,
                         render_points_as_spheres=True, label="Detected Narrowings")
    else:
        print("Nie wykryto żadnych zwężeń w analizowanej aorcie.")
    ...
    return {
        'mesh': mesh,
        'centerline': smoothed_coords,
        'narrowings': narrowing_segments,
        'skeleton': skeleton_cloud
    }


# ===== GŁÓWNE WYKONANIE =====
if __name__ == "__main__":
    # Ścieżka do pliku wejściowego NRRD
    nrrd_path = r"C:\Users\student.VIRMED\Desktop\Slicer_JM\CADRADS 5\Segmentation_left.nrrd"

    # 1. Konwersja NRRD → VTP
    vtp_path = convert_nrrd_to_vtp(nrrd_path)

    # 2. Analiza centerline i wizualizacja
    results = analyze_centerline(vtp_path)