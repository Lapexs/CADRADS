...
# Add these at the beginning of your script, right after imports
stenosis_actors = []
approved_regions = []
approved_stenosis_values = []
...
left_mask, left_skeleton, left_points, left_dist_map, left_spacing = load_artery(
    r"C:\Users\student.VIRMED\Desktop\Slicer_JM\CADRADS 5\Segmentation_left.nrrd", "Left")
right_mask, right_skeleton, right_points, right_dist_map, right_spacing = load_artery(
    r"C:\Users\student.VIRMED\Desktop\Slicer_JM\CADRADS 5\Segmentation_right.nrrd", "Right")
...
def find_paths(artery, G, points, selected_points, visual_objects, dist_map, skeleton, spacing):
    global stenosis_actors, approved_regions, approved_stenosis_values

    # Clear previous stenosis data
    stenosis_actors.clear()
    approved_regions.clear()
    approved_stenosis_values.clear()
    ...
    try:
        ...
        for region in regions1:
            ...
            sten_sphere.info = {
                ...
                'path': 1,
                'region': region,
                'diameter_data': diams1,
                'point_index': path1[region['max_stenosis_idx']]
            }
            plt.add(sten_sphere)
            visual_objects.append(sten_sphere)
            stenosis_actors.append(sten_sphere)
            approved_regions.append(region)
            approved_stenosis_values.append(region['max_stenosis'])
    ...
    if artery == 'left':
        try:
            ...
            for region in regions2:
                ...
                sten_sphere = vedo.Sphere(pos=sten_point, r=1.5, c='red')
                sten_sphere.info = {  # Dodajemy informacje o zwężeniu
                    'artery': artery,
                    'path': 2,
                    'region': region,
                    'diameter_data': diams2,
                    'point_index': path2[region['max_stenosis_idx']]
                }
                plt.add(sten_sphere)
                visual_objects.append(sten_sphere)
                stenosis_actors.append(sten_sphere)
...
def stenosis_click_handler(event):
    global stenosis_actors, approved_regions, approved_stenosis_values

    if not event.actor:
        return

    clicked_actor = event.actor
    if hasattr(clicked_actor, 'info') and clicked_actor in stenosis_actors:
        idx = stenosis_actors.index(clicked_actor)
        stenosis_info = clicked_actor.info
        artery = stenosis_info['artery']
        max_stenosis = approved_stenosis_values[idx]

        # Remove from visualization
        clicked_actor.pickable(False)
        clicked_actor.visible(False)
        plt.remove(clicked_actor)

        # Remove from lists
        stenosis_actors.pop(idx)
        approved_regions.pop(idx)
        approved_stenosis_values.pop(idx)

        print(f"Removed stenosis in {artery.upper()} artery: {max_stenosis:.1f}%")
        plt.render()


def confirm_stenosis():
    global stenosis_actors, approved_regions, approved_stenosis_values

    print("\nPotwierdzone zwężenia:")
    for i, (region, sten) in enumerate(zip(approved_regions, approved_stenosis_values), 1):
        print(f"{i}. Zwężenie {sten:.1f}%: punkt {region['max_stenosis_idx']}, długość {region['length']}")

    # Możesz tu dodać zapis do pliku lub inne operacje na potwierdzonych zwężeniach
    return approved_regions, approved_stenosis_values



plt.add_callback('LeftButtonPress', handle_click)
plt.add_callback('LeftButtonPress', stenosis_click_handler)
