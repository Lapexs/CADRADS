...
mesh = pv.read(r"output.vtp") #your path here
...
min_narrowing_length = 4     # Minimalna liczba punktów w zwężeniu
...
def detect_narrowings(centerline_indices, distances, derivative_threshold=0.045, min_length=3):
    """
    Wykrywa potencjalne zwężenia na podstawie zmian odległości od ścian (promieni).
    :param centerline_indices: indeksy punktów wzdłuż ścieżki centralnej
    :param distances: odległości punktów od ściany
    :param derivative_threshold: minimalna wartość spadku i wzrostu promienia (pochodna), by uznać za zwężenie
    :param min_length: minimalna liczba punktów w zwężeniu
    """
...
# === Detekcja zwężeń
narrowing_segments = detect_narrowings(path_indices, distances)
print("Liczba wykrytych zwężeń:", len(narrowing_segments))
print("Łączna liczba punktów w zwężeniach:", sum(end - start for start, end in narrowing_segments))
...
# Mapujemy punkty skeletonu, które należą do wykrytych zwężeń
...
# === Wizualizacja zwężeń jako czerwona chmura punktów
narrowing_cloud = skeleton_cloud.threshold(value=0.5, scalars="narrowing")
plotter.add_mesh(narrowing_cloud, color="yellow", point_size=6, render_points_as_spheres=True, label="Detected Narrowings")
plotter
